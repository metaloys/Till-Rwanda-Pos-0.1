...
export type Sale = { 
  ...
  is_returned: boolean; // FIX: Was missing, now required
  shop_id: string; 
  cashier_id?: string; // Added cashier_id
};
...
export type CreditPayment = { id: number; created_at: string; customer_id: number; payment_date: string; amount: number; payment_method: string; recorded_by_id: string; shop_id: string; };
...
export type Profile = {
  ...
  is_active: boolean; // This comes from the JOIN with shops table
  trial_ends_at: string | null; // This comes from the JOIN with shops table
};