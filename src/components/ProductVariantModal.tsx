...
export default function ProductVariantModal({
  ...
  const resetFormState = () => {
    ...
    // Use Ref for safe file input clearing
  ...
  const startEditing = (variant: ProductVariant) => {
    ...
    // Reset file input value on edit start
    if (fileInputRef.current) {
        fileInputRef.current.value = '';
    }
  ...
  const handleFormSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      alert(`Error processing variant: ${error.message}`);
    } finally {
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div id="variant-form" className="lg:col-span-1 rounded-lg border border-dashed border-gray-300 p-4">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-3">
              ...
              {/* --- IMAGE SECTION --- */}
               <div>
                  <label htmlFor="image-upload" className="label-style flex items-center">
                      <Upload className="mr-2 h-4 w-4" /> Variant Image (Opt)
                  </label>
                  {currentImageUrl && !imageFile && (
                    <div className="mt-2 text-xs text-gray-500 flex items-center space-x-2">
                       <img src={currentImageUrl} alt="Current" className="h-8 w-8 rounded object-cover" />
                       <span>Image currently set. Choose new file to replace.</span>
                    </div>
                  )}
                  <input
                    id="image-upload" type="file" accept="image/*"
                    ref={fileInputRef} 
                    className="input-field file:mr-4 file:py-1 file:px-2 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-gray-100 file:text-gray-700 hover:file:bg-gray-200"
                        ...
                        {variants.map((variant) => (
                            <tr key={variant.id}>
                                <td className="td-style">
                                    {variant.image_url ? (
                                        <img src={variant.image_url} alt={variant.name || 'Variant'} className="h-10 w-10 rounded object-cover" />
                                    ) : (
                                        <div className="h-10 w-10 rounded bg-gray-200 flex items-center justify-center text-xs text-gray-600">No Img</div>
                                    )}
                                </td>
                                ...
                                <td className="td-style text-right whitespace-nowrap">
                                    <button onClick={() => startEditing(variant)} disabled={isProcessing} className="action-button bg-yellow-100 text-yellow-700 hover:bg-yellow-200">
