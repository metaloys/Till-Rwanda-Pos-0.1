...
import RestockModal from './RestockModal'; // 1. IMPORT THE NEW MODAL
...
export default function ProductVariantModal({
  ...
  const handleDeleteVariant = async (variantId: number) => {
    ...
    setIsProcessing(true);
    const deletePromise = supabase.from('product_variants').delete().eq('id', variantId);
  ...
  const handleConfirmRestock = async (amountToAdd: number) => {
    if (!selectedVariant) return toast.error("No variant selected.");
    ...
    toast.promise(restockPromise, {
        ...
        success: () => {
            if (product) fetchVariants(product.id);
            onVariantUpdate();
            setShowRestockModal(false);
            setIsProcessing(false);
            return `${amountToAdd} units added to ${selectedVariant.name}.`;
        },
        error: (err) => {
            setIsProcessing(false);
            return `Error: ${err.message}`;
            ...
            <div id="variant-form" className="lg:col-span-1 rounded-lg border border-dashed border-gray-300 p-4">
              ...
              <form onSubmit={handleFormSubmit} className="space-y-3">
                ...
                <div>
                    ...
                    {currentImageUrl && !imageFile && ( <div className="mt-2 text-xs text-gray-500 flex items-center space-x-2"> <img src={currentImageUrl} alt="Current" className="h-8 w-8 rounded object-cover" /> <span>Image currently set.</span> </div> )}
                    <input id="image-upload" type="file" accept="image/*" ref={fileInputRef} className="input-field file:mr-4 file:py-1 file:px-2 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-gray-100 file:text-gray-700 hover:file:bg-gray-200" onChange={handleFileChange} disabled={isProcessing} />
                    {imageFile && <p className="mt-1 text-xs text-gray-500 truncate font-semibold">Ready to upload: {imageFile.name}</p>}
                  </div>
                              ...
                              {variants.map((variant) => (
                                  <tr key={variant.id}>
                                      ...
                                      <td className="td-style text-right whitespace-nowrap">
                                          <button onClick={() => handleOpenRestockModal(variant)} disabled={isProcessing} className="action-button bg-green-100 text-green-700 hover:bg-green-200"><PlusCircle className="h-4 w-4" /></button>
                      ...
                      {variants.map((variant) => (
                          <div key={variant.id} className="rounded-lg border bg-white p-4 shadow-sm">
                              ...
                              <div className="mt-4 grid grid-cols-3 gap-2 border-t pt-3">
                                  <button onClick={() => handleOpenRestockModal(variant)} disabled={isProcessing} className="action-button justify-center bg-green-100 text-green-700 hover:bg-green-200"><PlusCircle className="mr-1.5 h-4 w-4" /> Restock</button>
