...
export default function SuperAdminDashboard() {
    ...
    async function fetchPlatformMetrics() { setLoading(true); const { data, error } = await supabase.from('shops').select('id, name, is_active, trial_ends_at').order('created_at', { ascending: false }); if (error) { console.error("Error fetching shops:", error.message); alert(error.message); } else if (data) { const now = new Date().getTime(); const shopMetrics: ShopMetric[] = data.map(shop => { const trialEnd = new Date(shop.trial_ends_at).getTime(); const diffTime = trialEnd - now; const daysLeft = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); return { ...shop, days_left: daysLeft, } as ShopMetric; }); setShops(shopMetrics); } setLoading(false); }
    useEffect(() => { fetchPlatformMetrics(); }, []);
