...
export default function Reports({ shopId }: ReportsProps) {
  ...
  async function fetchReportData(date: Date) {
    ...
    setTopProducts([]); setInventoryValue(0); setSlowProducts([]);
    ...
    const [salesResult, expensesResult, topProductsResult, inventoryResult, slowProductsResult] = await Promise.all([
    ...
    if (slowProductsResult.data) {
        setSlowProducts(slowProductsResult.data);
    }
    
    if (salesResult.error || expensesResult.error || topProductsResult.error || inventoryResult.error || slowProductsResult.error) {
      console.error('Error fetching report data:', salesResult.error?.message || expensesResult.error?.message || topProductsResult.error?.message || inventoryResult.error?.message || slowProductsResult.error?.message);
    }
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg bg-white p-4 shadow mb-6">
        <div className="flex items-center justify-between gap-4">
          <label htmlFor="report-date" className="block text-sm font-medium text-gray-700">Daily Report Date:</label>
      ...
      <div className="rounded-lg bg-white p-6 shadow">
        <h2 className="flex items-center text-lg font-semibold text-gray-900 border-b pb-3">
            <Archive className="mr-2 h-5 w-5 text-gray-500" />
            Slow-Moving Inventory (No Sales in 90 Days)
        </h2>
        <div className="mt-4 flow-root overflow-x-auto">
            {loading && slowProducts.length === 0 ? (
                <p className="py-4 text-center text-gray-500">Checking for dusty stock...</p>
            ) : slowProducts.length === 0 ? (
                <p className="py-4 text-center text-green-600 font-medium">No slow-moving items found. Everything has sold at least once in 90 days!</p>
            ) : (
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            <th className="th-style">Product</th>
                            <th className="th-style">Variant</th>
                            <th className="th-style">In Stock</th>
                            <th className="th-style">Value (RWF)</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200 bg-white">
                        {slowProducts.map((product) => (
                            <tr key={product.variant_id}>
                                <td className="td-style font-medium text-gray-900">{product.parent_product_name}</td>
                                <td className="td-style text-sm text-gray-500">{product.name}</td>
                                <td className="td-style font-semibold text-red-600">{product.stock_quantity}</td>
