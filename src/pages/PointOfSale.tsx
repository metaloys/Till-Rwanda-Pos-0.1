...
export default function PointOfSale({ shopId, profile, userRole }: PointOfSaleProps) {
  ...
  async function fetchVariants() {
    ...
    const { data, error } = await supabase.from('product_variants').select('*, products(name, category, id)').gt('stock_quantity', 0).order('name', { ascending: true });
    ...
    <div className="relative grid grid-cols-1 md:grid-cols-12 gap-6 h-[calc(100vh-9rem)]">
      ...
      <div className="md:col-span-5 flex flex-col rounded-lg bg-white dark:bg-slate-800 p-4 shadow-lg order-1 md:order-2 md:h-full">
        ...
        <div className="mt-4 flex-1 overflow-y-auto divide-y divide-slate-200 dark:divide-slate-700">
           <div className="flex justify-end pr-2 text-xs font-semibold text-slate-500 dark:text-slate-400">Price | Final Price</div>
          {cart.length === 0 ? (<p className="flex h-full items-center justify-center text-slate-500 dark:text-slate-400">Cart is empty.</p>) : (
            cart.map((item) => (
              <div key={item.id} className="flex items-center justify-between py-3">
                ...
                <div className='flex items-center gap-2'><p className="font-medium text-slate-800 dark:text-slate-200">{(item.price * (1 - cartDiscountPercent / 100) * item.quantity).toLocaleString()} RWF</p><button onClick={() => removeFromCart(item.id)} className="text-red-500 hover:text-red-700"><Trash2 className="h-4 w-4" /></button></div>
              </div>
            ))
          )}
        </div>
