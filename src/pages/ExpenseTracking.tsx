...
import { ReceiptText, Upload, Repeat2, Calendar, Tag, DollarSign } from 'lucide-react'; // Added icons
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="mt-4 space-y-4 md:hidden">
            {expenses.length === 0 ? (<p className="py-10 text-center text-gray-500">No expenses recorded yet.</p>) : (
              expenses.map((expense) => (
                <div key={expense.id} className="rounded-lg border bg-white p-4 shadow-sm">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="font-bold text-lg text-red-700">{expense.amount.toLocaleString()} RWF</div>
                  </div>
                  <div className="mt-2 space-y-1.5 text-sm text-gray-600">
                    ...
                    <div className="flex items-center">
                      {expense.is_recurring ? (
                        <div className="flex items-center font-medium text-purple-700"><Repeat2 className="mr-2 h-4 w-4" /> <span className="capitalize">{expense.recurrence_interval}</span></div>
                      ) : (
                  ...
                  {expense.receipt_url && (
                    <button onClick={() => handleViewReceipt(expense.receipt_url as string)} className="action-button mt-3 w-full justify-center bg-purple-100 text-purple-700 hover:bg-purple-200">
                      <ReceiptText className="mr-1.5 h-4 w-4" /> View Receipt
                    </button>
                  )}
                </div>
              ))
