...
import { List, Undo2, User, Calendar, Tag, DollarSign, CheckCircle, XCircle } from 'lucide-react';
...
export default function SalesHistory({ shopId }: SalesHistoryProps) {
  ...
  return (
    <div className="rounded-lg bg-white p-4 md:p-6 shadow">
      ...
      {loading ? (
        <p className="py-10 text-center text-gray-500">Loading sales history...</p>
      ) : (
        <>
          ...
          {/* --- MOBILE CARD LIST (Visible on mobile, hidden on md and up) --- */}
          <div className="mt-4 space-y-4 md:hidden">
            {sales.length === 0 ? (
              <p className="py-10 text-center text-gray-500">No sales recorded yet.</p>
            ) : (
              sales.map((sale) => (
                <div key={sale.id} className="rounded-lg border bg-white p-4 shadow-sm">
                  <div className="flex items-center justify-between border-b pb-2">
                    ...
                    <div className={`text-sm font-bold ${sale.is_returned ? 'text-red-600' : 'text-green-600'}`}>
                      {sale.is_returned ? 'RETURNED' : 'COMPLETED'}
                    </div>
                  </div>
                  <div className="mt-2 space-y-2">
                    ...
                    <div className="flex items-center justify-between text-sm text-gray-600">
                      ...
                      <span className="flex items-center"><Calendar className="mr-1.5 h-4 w-4" /> {new Date(sale.created_at).toLocaleDateString()}</span>
                    </div>
                    {sale.transaction_reference && (
                      <div className="flex items-center text-sm text-gray-500">
                        <Tag className="mr-1.5 h-4 w-4" /> Ref: {sale.transaction_reference}
                      </div>
                    )}
                  </div>
                  {!sale.is_returned && (
                    <button
                      ...
                      className="action-button mt-3 w-full justify-center bg-red-100 text-red-700 hover:bg-red-200"
                    >
