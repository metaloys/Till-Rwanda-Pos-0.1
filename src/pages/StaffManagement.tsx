...
import { Users, AlertTriangle, Loader2, Mail, Shield, Calendar, Trash2, UserCheck, UserX } from 'lucide-react'; // Added UserCheck/UserX
...
export default function StaffManagement({ userRole, profile }: StaffManagementProps) {
  ...
  // --- NEW: Handle Deactivate/Reactivate ---
  const handleToggleActive = async (targetProfile: Profile, activate: boolean) => {
    ...
    try {
        ...
        // This command revokes the user's current session, forcing immediate logout
        const { error: signOutError } = await supabase.auth.admin.signOutUser(targetProfile.id);
        if (signOutError) console.warn(`Could not force sign out user ${targetProfile.id}: ${signOutError.message}`);
  ...
  return ( 
    <div className="rounded-lg bg-white p-4 md:p-6 shadow space-y-6">
      <h2 className="flex items-center text-lg font-semibold text-slate-900 border-b pb-3">
        <Users className="mr-2 h-5 w-5" /> Staff Management: {profile.shop_name || 'N/A'}
      </h2>
      <div className="border border-dashed border-blue-300 rounded-lg p-4 bg-blue-50">
        <h3 className="font-semibold text-lg mb-3 flex items-center text-blue-800">
          <Mail className="h-5 w-5 mr-2" /> Invite New Staff
        </h3>
          ...
          <>
            ...
            <div className="mt-4 hidden md:block flow-root overflow-x-auto">
              <table className="min-w-full divide-y divide-slate-200">
                <thead className="bg-slate-50"><tr><th className="th-style">Name / Email</th><th className="th-style">Member Since</th><th className="th-style">Current Role</th><th className="th-style">Status</th><th className="th-style">Actions</th></tr></thead>
                <tbody className="divide-y divide-slate-200 bg-white">
                  {staffProfiles.length === 0 ? (<tr><td colSpan={5} className="td-style text-center text-gray-500">No other staff members found.</td></tr>) : (staffProfiles.map((staffProfile) => (<tr key={staffProfile.id}><td className="td-style font-medium text-slate-900">{staffProfile.full_name || staffProfile.id}<p className="text-xs text-slate-500">{staffProfile.id}</p></td><td className="td-style text-sm text-slate-500">{new Date(staffProfile.created_at).toLocaleDateString()}</td><td className={`td-style font-semibold capitalize ${staffProfile.role === 'owner' ? 'text-purple-600' : staffProfile.role === 'manager' ? 'text-indigo-600' : 'text-blue-600'}`}>{staffProfile.role}</td><td className="td-style">
                        ...
                        {staffProfile.status === 'active' ? (
                            <button onClick={() => handleToggleActive(staffProfile, false)} disabled={isProcessing} className="action-button bg-red-100 text-red-700 hover:bg-red-200 ml-2">
                                <UserX className="h-4 w-4" /> Deactivate
                            </button>
                        ) : (
                             <button onClick={() => handleToggleActive(staffProfile, true)} disabled={isProcessing} className="action-button bg-green-100 text-green-700 hover:bg-green-200 ml-2">
                ...
                staffProfiles.map((staffProfile) => (
                  <div key={staffProfile.id} className="rounded-lg border bg-white p-4 shadow-sm">
                    <div className="flex items-center justify-between">
                        ...
                        <div className={`px-2 py-0.5 rounded-full text-xs font-medium ${staffProfile.status === 'active' ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800'}`}>
                            {staffProfile.status.toUpperCase()}
                        </div>
                    </div>
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-2 border-t pt-3">
                      <div>
                        ...
                        <select id={`role-${staffProfile.id}`} value={staffProfile.role} onChange={(e) => handleRoleChange(staffProfile, e.target.value as UserRole)} disabled={isProcessing || staffProfile.role === 'owner' || userRole !== 'owner'} className="input-field py-2 text-sm disabled:bg-slate-100 disabled:opacity-80">
                          {ROLE_OPTIONS.map((role) => ( <option key={role} value={role}>{role.charAt(0).toUpperCase() + role.slice(1)}</option> ))}
                        </select>
                      </div>
                      {staffProfile.status === 'active' ? (
                          <button onClick={() => handleToggleActive(staffProfile, false)} disabled={isProcessing} className="action-button bg-red-100 text-red-700 hover:bg-red-200">
                              <UserX className="mr-1.5 h-4 w-4" /> Deactivate
                          </button>
                      ) : (
                          <button onClick={() => handleToggleActive(staffProfile, true)} disabled={isProcessing} className="action-button bg-green-100 text-green-700 hover:bg-green-200">
