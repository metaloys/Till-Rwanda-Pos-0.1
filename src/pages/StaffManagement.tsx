...
import { Users, AlertTriangle, Loader2, Mail } from 'lucide-react';
...
export default function StaffManagement({ userRole, profile }: StaffManagementProps) {
  ...
  const handleRoleChange = async (targetProfile: Profile, newRole: UserRole) => { 
    if (!confirm(`Change ${targetProfile.full_name}'s role to ${newRole.toUpperCase()}?`)) { 
      return; 
    } 
    ...
    try { 
      ...
      alert(`Role updated for ${targetProfile.full_name}.`); 
      fetchStaffProfiles(); 
    } catch (error: any) { 
