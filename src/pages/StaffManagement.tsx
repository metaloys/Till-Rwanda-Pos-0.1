...
import { Users, AlertTriangle, Loader2, Mail, Shield, Calendar, Trash2, UserCheck, UserX } from 'lucide-react';
...
export default function StaffManagement({ userRole, profile }: StaffManagementProps) {
  ...
  const handleRoleChange = async (targetProfile: Profile, newRole: UserRole) => { 
    // This is a simple select change, no high-stakes confirmation needed
  ...
  // --- 5. UPDATED FUNCTIONS TO OPEN MODAL ---
  ...
  // --- 6. NEW: Master function to handle confirmation ---
  const onConfirmAction = async () => {
    ...
    if (action === 'delete') {
        ...
        promise = supabase.functions.invoke('delete-staff', {
            body: JSON.stringify({ userIdToDelete: targetProfile.id }),
        ...
        promise = supabase.functions.invoke('toggle-staff-status', {
  ...
  return ( 
    <>
              ...
              <div className="mt-4 hidden md:block flow-root overflow-x-auto"><table className="min-w-full divide-y divide-slate-200"><thead className="bg-slate-50"><tr><th className="th-style">Name / Email</th><th className="th-style">Member Since</th><th className="th-style">Current Role</th><th className="th-style">Status</th><th className="th-style">Actions</th></tr></thead><tbody className="divide-y divide-slate-200 bg-white">{staffProfiles.length === 0 ? (<tr><td colSpan={5} className="td-style text-center text-gray-500">No other staff members found.</td></tr>) : (
                staffProfiles.map((staffProfile) => (<tr key={staffProfile.id}><td className="td-style font-medium text-slate-900">{staffProfile.full_name || staffProfile.id}<p className="text-xs text-slate-500">{staffProfile.id}</p></td><td className="td-style text-sm text-slate-500">{new Date(staffProfile.created_at).toLocaleDateString()}</td><td className={`td-style font-semibold capitalize ${staffProfile.role === 'owner' ? 'text-purple-600' : staffProfile.role === 'manager' ? 'text-indigo-600' : 'text-blue-600'}`}>{staffProfile.role}</td><td className="td-style">
                  ...
                  {staffProfile.status === 'active' ? (
                      <button onClick={() => handleToggleActive(staffProfile, false)} disabled={isProcessing || staffProfile.role === 'owner'} className="action-button bg-red-100 text-red-700 hover:bg-red-200 disabled:opacity-50"><UserX className="h-4 w-4" /></button>
                  ) : (
                      <button onClick={() => handleToggleActive(staffProfile, true)} disabled={isProcessing || staffProfile.role === 'owner'} className="action-button bg-green-100 text-green-700 hover:bg-green-200 disabled:opacity-50"><UserCheck className="h-4 w-4" /></button>
                  )}
                  ...
                  staffProfiles.map((staffProfile) => (
                    <div key={staffProfile.id} className="rounded-lg border bg-white p-4 shadow-sm">
                      ...
                      <div className="mt-3 grid grid-cols-2 gap-2 border-t pt-3">
                        {staffProfile.status === 'active' ? (
                            <button onClick={() => handleToggleActive(staffProfile, false)} disabled={isProcessing || staffProfile.role === 'owner'} className="action-button justify-center bg-red-100 text-red-700 hover:bg-red-200 disabled:opacity-50"><UserX className="mr-1.5 h-4 w-4" /> Deactivate</button>
                        ) : (
                            <button onClick={() => handleToggleActive(staffProfile, true)} disabled={isProcessing || staffProfile.role === 'owner'} className="action-button justify-center bg-green-100 text-green-700 hover:bg-green-200 disabled:opacity-50"><UserCheck className="mr-1.5 h-4 w-4" /> Reactivate</button>
                        )}
                        <button onClick={() => handleDeleteStaff(staffProfile)} disabled={isProcessing || staffProfile.role === 'owner'} className="action-button justify-center bg-gray-100 text-gray-700 hover:bg-gray-200 disabled:opacity-50"><Trash2 className="mr-1.5 h-4 w-4" /> Delete</button>
                      </div>
                    </div>
                  ))
                )}
      ...
      >
        <p>Are you sure you want to {confirmState?.action} <span className="font-bold">{confirmState?.profile?.full_name}</span>?</p>
        {confirmState?.action === 'delete' && <p className="mt-2 text-sm text-red-600">This action is permanent and cannot be undone.</p>}
        {confirmState?.action === 'deactivate' && <p className="mt-2 text-sm text-yellow-700">This will immediately log them out of all devices.</p>}
      </ConfirmModal>
    </>
  );
}