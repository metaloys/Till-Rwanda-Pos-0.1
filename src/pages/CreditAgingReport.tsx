...
import { History } from 'lucide-react'; // FIX: Removed RefreshCw

type DebtorInfo = Customer & { last_credit_sale_date: string | null; };
...
export default function CreditAgingReport() {
  ...
  async function fetchCreditReport() {
    ...
    for (const customer of debtors) {
      ...
      if (lastSaleError && lastSaleError.code !== 'PGRST116') {
        console.error(`Error fetching last sale for ${customer.id}:`, lastSaleError.message);
  ...
  return (
    <div className="rounded-lg bg-white p-6 shadow">
      <h2 className="flex items-center text-lg font-semibold text-gray-900"><History className="mr-2 h-5 w-5 text-orange-600" />Credit Aging (Simplified)</h2>
       <p className="mt-1 text-sm text-gray-500">Debtors ordered by their last credit purchase.</p>
      <button onClick={fetchCreditReport} disabled={loading} className="mt-2 text-xs text-blue-600 hover:underline disabled:opacity-50">{loading ? 'Refreshing...' : 'Refresh Report'}</button>
      <div className="mt-4 flow-root">
        {loading && debtorReport.length === 0 ? (<p>Loading...</p>) : (
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50"><tr><th className="th-style">Name</th><th className="th-style">Phone</th><th className="th-style">Balance (RWF)</th><th className="th-style">Last Credit Sale</th><th className="th-style">Approx. Age</th></tr></thead>
            <tbody className="divide-y divide-gray-200 bg-white">
              {debtorReport.length === 0 ? (
                ...
                debtorReport.map((customer) => (
                  <tr key={customer.id}><td className="td-style font-medium text-gray-900">{customer.name}</td><td className="td-style text-sm text-gray-500">{customer.phone || 'N/A'}</td><td className="td-style font-semibold text-red-600">{customer.credit_balance.toLocaleString()} RWF</td><td className="td-style text-sm text-gray-500">{customer.last_credit_sale_date ? new Date(customer.last_credit_sale_date).toLocaleDateString() : 'N/A'}</td><td className="td-style text-sm text-gray-500">{calculateDaysAgo(customer.last_credit_sale_date)}</td></tr>
                ))
              )}
            </tbody>
          </table>
